...
| 决策 | 结论 |
|------|------|
| 产品定位 | 本地优先 + 私有 AI（三层：无模型 / 本地模型 / BYOK 云端） |
| 环境变量（F3） | **做**，定位「可复用 + AI 变量注入基础」 |
| 测试脚本（F4） | **降级**，不做完整 JS 沙箱，改轻量断言 + AI 生成 + CLI/CI 导出 |
| 预请求链（F8） | **新增**，声明式登录→token + 密码 sha1/aes 加密 |
| 第一步 | 状态纠偏 + UX 审计（先做，产出可验证清单） |
...
> 不做完整 JS 沙箱 + pre-request/test script；预请求动态签名/加密能力并入 F8。

| ID | 功能 | 优先级 | 预估工作量 | 说明 |
|----|------|--------|------------|------|
| F4.1 | 响应断言（轻量） | ⭐⭐ | 1周 | 状态码/Header/Body/JSONPath，UI 配置 |
| F4.2 | 断言生成（AI） | ⭐⭐ | 1周 | AI 根据响应样本生成断言 |
| F4.3 | 批量运行 | ⭐⭐ | 2周 | 集合级批量执行 + 报告 |
| F4.4 | CLI / CI 导出 | ⭐⭐⭐ | 2周 | 集合+断言导出为可运行脚本 |
...
| ID | 功能 | 优先级 | 预估工作量 | 说明 |
|----|------|--------|------------|------|
| F8.1 | 认证（Auth）配置 | ⭐⭐⭐ | 1周 | Bearer / Basic / API Key |
| F8.2 | 预请求链（登录→token） | ⭐⭐⭐ | 2周 | 前置请求、JSONPath/正则/Header 提取、变量注入 |
| F8.3 | 变量转换（sha1/aes/hmac） | ⭐⭐⭐ | 2周 | 声明式哈希/加密/签名，替代 JS 脚本 |

### 六、AI 助手（三层）

| ID | 层 | 优先级 | 预估工作量 | 说明 |
|----|----|--------|------------|------|
| F9.1 | Tier 0 无模型 | ⭐⭐⭐ | 2周 | OpenAPI/Swagger 导入生成请求/collection |
| F9.2 | Tier 1 本地模型 | ⭐⭐ | 2周 | Ollama/LM Studio：解释响应、生成断言、自然语言建请求 |
| F9.3 | Tier 2 BYOK 云端 | ⭐ | 1周 | 默认关闭，用户自填 key |
