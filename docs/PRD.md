...
### 四、测试与断言功能 🟡 大部分完成（F4.1/F4.2/F4.4 ✅；F4.3 批量运行 ⏳）
...
> - F4.1 / F4.4 已由 M8.4 交付（v0.12.0，2026-08-28）；F4.2 随 M8.5 交付（v0.13.0，2026-08-31）；F4.3 批量运行视需求排期。

| ID | 功能 | 状态 | 需求描述 | 验收标准 |
|----|------|------|----------|----------|
| F4.1 | 响应断言（轻量） | ✅ | 状态码 / Header / Body / JSONPath / 响应时间断言 | 无需写代码，UI 配置断言规则（M8.4 / v0.12.0，2026-08-28） |
| F4.2 | 断言生成（AI） | ✅ | 由 AI 根据响应样本生成断言 | 一键生成、可编辑、可保存（M8.5 / v0.13.0，2026-08-28 调整排期，2026-08-31 交付） |
...
| F5.9 | 多语言（界面国际化） | ✅ | 全量字符串抽取 + 语言切换（跟随系统 / English / 中文），消除界面中英文混杂（M8.8 / v0.16.0，2026-09-03） | 任一语种下界面无中英混杂；切换即时生效并持久化 |
...
- [x] `en` / `zh` 任一语种下界面无中英混杂（test-mode 双语言截图审计：主界面 + 设置对话框，`set_language` 指令运行时切换验证）
- [x] 语言切换即时生效、重启后保持（`AppSettingsDialog` widget 测试验证持久化与 localeProvider 联动）；`system` 档跟随 OS 语言
- [x] 全量测试 + design_guard 通过（1014 通过 / 2 跳过，2026-09-03）；亮/暗双主题截图无回归
...
| 层级 | 技术 | 用途 |
|------|------|------|
| 跨平台框架 | Flutter 3.35.x | UI 开发和跨平台支持 |
