...
| 2026-08-25 | v0.10.0-chain-logging | 预请求链可观测性（用户实测驱动）：`PreRequestChainRunner` 全生命周期日志——链启动（区分 send/dry-run）、每步方法+URL、步骤内未解析变量警告、状态码、提取命中/缺失的规则路径与目标变量名（只记变量名，不落值，密码/token 不进日志）；此前链执行与试运行完全无日志、步骤内未解析变量静默原文发送；test-mode 新增 `create_saved_request`（直接落指定 Collection，供链引用）与 `set_pre_request_chain`（步骤+提取规则+401 重跑）两条指令，链配置可全自动化；经真实 API + 本地 echo server 抓包实机验证：链→提取→变量解析上线全链路 |
| 2026-08-25 | v0.10.0-extract-input-commit | 修复预请求链提取规则输入被清空（用户录屏发现）：路径/目标变量输入框 `onChanged` 为空实现、仅回车提交，叠加 build 时强制回写持久态——点试运行触发重建即把输入打回原值，用户填写从未生效（试运行始终跑旧规则）；改为 `onChanged` 即时提交（集合设置对话框同组件一并受益）；新增回归测试 2 个（输入即提交 + 重建不回退）；全量 729 通过（2 跳过） |
| 2026-08-25 | v0.10.0-kv-input-commit | 修复 Params/Headers KV 行输入被 Send 清空（用户截图发现，与提取规则同族缺陷）：KV 行仅回车/onEditingComplete 提交 + build 强制回写 provider 旧值——未回车的输入在 Send 时既丢失也未参与发送；抽 `commitRow()` 改 `onChanged` 即时提交（key 自动完成逻辑保留）；排查全库输入框确认该缺陷模式仅此两处（Auth/Body/URL 栏/环境对话框/重命名/cURL 导入/fx 表单均为即时提交或动作域 controller）；新增回归测试（输入即提交 + 重建不清空）；全量 730 通过（2 跳过） |
