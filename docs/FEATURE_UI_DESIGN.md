...
| 功能                    | 屏幕/入口                    | 关键交互                       | 状态 |
| --------------------- | ------------------------ | -------------------------- | --- |
| 环境变量（M8.1）          | 顶部环境切换器 + 环境管理对话框        | 切换即解析；secret 脱敏/👁/复制      | ✅ v0.8.0 |
| 变量转换（F8.3）          | Headers/Body 行内值 + fx 菜单 | 点值 → 选算法 → 实时预览            | ✅ v0.10.0 |
| 预请求链（F8.2）          | 请求编辑器 Pre-request 页签          | 配置 login + 提取 + 注入         | ✅ v0.10.0 |
| 认证（F8.1）             | Auth Tab                 | 选类型 → 引用变量                 | ✅ v0.10.0 |
| 轻量断言（F4.1）          | 请求编辑器 Assertions 页签 + 响应区 Tests 页签 | 发送后自动跑、行级通过/失败  | ✅ v0.12.0 |
...
flowchart LR
    ...
    D -->|是| E[绿色汇总]
    D -->|否| F[红色 + 失败明细]
    E --> G[可导出 .hopp.json，hopp run 跑 CI]
    F --> G
```
...
> 进度状态见 [DEVELOPMENT_PLAN](./DEVELOPMENT_PLAN.md)；1–5 已完成（M8.5 / v0.13.0，2026-08-31），下一步 M8.8（多语言 F5.9）→ M8.9（Tier 2 BYOK）。

1. 环境变量（M8.1）—— 一切变量能力的地基 ✅
2. 预请求链 + 变量转换（F8）—— 直击登录/token/加密痛点 ✅
3. OpenAPI 导入（Tier 0）—— 无模型、零隐私成本、最强差异化 ✅
4. 轻量断言 + CLI（F4 降级）—— CI 与可被编排 ✅
5. AI 本地模型（Tier 1）→ BYOK 云端（Tier 2）—— 下一个里程碑 M8.5/M8.6
