...
| `set_ui_scale` | 设置界面文字缩放（`scale`: 0.8/0.9/1.0/1.25/1.5，F5.7；持久化生效，HiDPI 紧凑布局验收用） |
| `set_language` | 切换界面语言（`language`: system/en/zh，F5.9；即时生效 + 持久化，双语言截图审计用） |
| `open_app_settings` | 打开应用设置对话框（主题/语言/界面缩放，F5.9） |
