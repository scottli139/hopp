...
- **配置**：`AppSettings` HiveField 8-12（aiEnabled/aiProviderPreset/aiBaseUrl/aiModel/aiApiKey）；settings box 存 toJson Map、fromJson `?? default` 回退，老数据无需迁移；预设 Ollama `localhost:11434/v1` / LM Studio `localhost:1234/v1`。keychain 级密钥存储随 M8.9（Tier 2）。
...
- **测试**：新增 39——transforms 时间函数 18（各单位/秒毫秒自适应/非法输入/TZ 无关的 floor 期望值）+ epoch_annotation 12（format/scanLine/annotateLine，含字符串内数字与转义引号、秒级 +5 年缓冲防 10 位 id 误判）+ viewer widget 5（Full/Performance/Raw/开关/非 JSON）+ fx 菜单 widget 4（动态变量插入/date_floor 表单/date_add 校验）。交付时全量 989 通过（2 跳过）；试用修复再补 6（选区替换 + 管道去重回归 2、渲染不变量 3、Body 工具条 widget 1），最终全量 995 通过（2 跳过），design_guard 零新增，analyze 零 error/warning。
