...
| 2026-08-25 | v0.10.2-add-rule-button-fix | 修复「添加提取规则」按钮不符合设计规范（用户截图发现）：预请求链编辑器中手写 InkWell+Container 一次性按钮（tiny11 文字/13px 图标/textTertiary 前景/无 hover 反馈），与同屏「添加步骤」`AppButton.secondary` 规格全错位；替换为 `AppButton.secondary` 统一组件，两按钮视觉对齐；test-mode 实机截图亮/暗双主题目检通过；全量 730 通过（2 跳过），analyze 基线无新增 |
