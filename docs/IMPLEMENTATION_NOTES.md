...
- **配置**：`AppSettings` HiveField 8-12（aiEnabled/aiProviderPreset/aiBaseUrl/aiModel/aiApiKey）；settings box 存 toJson Map、fromJson `?? default` 回退，老数据无需迁移；预设 Ollama `localhost:11434/v1` / LM Studio `localhost:1234/v1`。keychain 级密钥存储随 M8.8（Tier 2）。
